...
export default function DynamicForm({
  ...
  async function handleFormSubmit(values: z.infer<typeof formSchema>) {
    try {
      ...
      setError(err?.message || (err as any).error || "An error occurred");

      toast({
        title: "Error",
        description: err?.message || (err as any).error || "Failed to submit form",
        variant: "destructive",
      });
    } finally {
  ...
  return (
    <Form {...form}>
      <form onSubmit={form.handleSubmit(handleFormSubmit)} className={`space-y-6  w-full ${className}`}>
                ...
                <FormField
                  ...
                  render={({ field: formField }) => (
                    <FormItem>
                      ...
                      <FormControl>
                        <PhotoInput single={field.single} name={formField.name} />
                      </FormControl>
                      <FormMessage />
                    </FormItem>
                  )}
        ...
        {children && typeof children === "function" ? children(form.control, form.getValues) : children}{" "}
